...
/**
 * This handles the initial part of the LTI 1.3 Resource Link Request and sends the browser back to
 * the platform to start the OAuth2 Implicit Flow.
 *
...
public class OIDCInitiatingLoginRequestResolver implements OAuth2AuthorizationRequestResolver {
  ...
  /** The Constant REGISTRATION_ID_URI_VARIABLE_NAME. */
  private static final String REGISTRATION_ID_URI_VARIABLE_NAME = "registrationId";

  /** The client registration repository. */
  private final ClientRegistrationRepository clientRegistrationRepository;

  /** The authorization request matcher. */
  private final AntPathRequestMatcher authorizationRequestMatcher;
  // The IMS LTI 1.3 Validator doesn't include = (%3D URL encoded) in state tokens.
  /** The state generator. */
  // private final StringKeyGenerator stateGenerator = new
  // Base64StringKeyGenerator(Base64.getUrlEncoder());
   ...
   *        requests
   */
  public OIDCInitiatingLoginRequestResolver(
   ...
   * @param request the request
   * @param registrationId the registration id
   * @param redirectUriAction the redirect uri action
   * @return the o auth 2 authorization request
   */
  private OAuth2AuthorizationRequest resolve(HttpServletRequest request, String registrationId,
    ...
    if (new AuthorizationGrantType("implicit")
    ...
    OAuth2AuthorizationRequest authorizationRequest =
        builder.clientId(clientRegistration.getClientId())
            ...
            .state(this.stateGenerator.generateKey()).additionalParameters(additionalParameters)
  ...
  private String resolveRegistrationId(HttpServletRequest request) {
    if (this.authorizationRequestMatcher.matches(request)) {
      return this.authorizationRequestMatcher.extractUriTemplateVariables(request)
  ...
  private String expandRedirectUri(HttpServletRequest request,
    ...
    // Used in -> CommonOAuth2Provider.DEFAULT_REDIRECT_URL =
    // "{baseUrl}/{action}/oauth2/code/{registrationId}"
    Map<String, String> uriVariables = new HashMap<>();
    uriVariables.put("registrationId", clientRegistration.getRegistrationId());
    String baseUrl = UriComponentsBuilder.fromHttpUrl(UrlUtils.buildFullRequestUrl(request))
        .replaceQuery(null).replacePath(request.getContextPath()).build().toUriString();

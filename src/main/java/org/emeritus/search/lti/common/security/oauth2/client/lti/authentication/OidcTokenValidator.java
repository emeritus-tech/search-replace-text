...
/**
 * This needs to be update for the launch flow for IMS Sec 1.0 We should refactor this so that it
 * implements org.springframework.security.oauth2.core.OAuth2TokenValidator
 *
 * @author Rob Winch
...
final class OidcTokenValidator {
  ...
  /** The Constant INVALID_ID_TOKEN_ERROR_CODE. */
  ...
  static void validateIdToken(OidcIdToken idToken, ClientRegistration clientRegistration) {
    ...
    // 10. The iat Claim can be used to reject tokens that were issued too far away from the current
    // time,
    ...
    // These are the LTI Claims that we check
    // https://www.imsglobal.org/spec/lti/v1p3/#required-message-claims

    String ltiVersion =
        idToken.getClaimAsString("https://purl.imsglobal.org/spec/lti/claim/version");
    if (!"1.3.0".equals(ltiVersion)) {
      throwInvalidIdTokenException("Must be LTI 1.3.0 version claim in token.");
    }

    String messageType =
        idToken.getClaimAsString("https://purl.imsglobal.org/spec/lti/claim/message_type");
    if (messageType == null || messageType.isEmpty()) {
      throwInvalidIdTokenException("Message type claim missing from token.");
    }

    List<String> roles =
        idToken.getClaimAsStringList("https://purl.imsglobal.org/spec/lti/claim/roles");
    if (roles == null) {
    ...
    String deploymentId =
        idToken.getClaimAsString("https://purl.imsglobal.org/spec/lti/claim/deployment_id");
  ...
  /**
   * Throw invalid id token exception.
   */
  ...
  private static void throwInvalidIdTokenException(String message) {
    ...
    throw new OAuth2AuthenticationException(invalidIdTokenError,
        (message != null) ? message : invalidIdTokenError.toString());
  }

  /**
   * Instantiates a new oidc token validator.
   */
